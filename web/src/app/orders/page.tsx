...
export default function OrdersPage() {
  ...
  const filteredShipments = shipments.filter(s => {
    ...
    if (searchTerm) {
      ...
      return (s as any).reference_number?.toLowerCase().includes(search) ||
        (s as any).customer_name?.toLowerCase().includes(search);
    }
    return true;
  });
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-500 mt-1">Manage import and export orders</p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="bg-white rounded-xl shadow p-4"><p className="text-sm text-gray-500">Total Orders</p><p className="text-2xl font-bold">{stats.total}</p></div>
      ...
      <div className="bg-white rounded-xl shadow p-4 flex flex-wrap gap-4">
        ...
        <input type="text" placeholder="Search orders..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="px-4 py-1.5 border rounded-lg text-sm w-64" />
      ...
      {selectedShipment && (
        <>
          ...
          <EditShipmentModal isOpen={isEditModalOpen} onClose={() => { setIsEditModalOpen(false); setSelectedShipment(null); }} onSave={() => { fetchShipments(); setIsEditModalOpen(false); setSelectedShipment(null); }} shipment={selectedShipment} />
